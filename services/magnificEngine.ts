...
// apart and silently diverge on prompt logic or the Magnific API contract.
...
export interface MagnificGenerationError extends Error {
  status: number;
}

function makeError(message: string, status: number): MagnificGenerationError {
  const err = new Error(message) as MagnificGenerationError;
  err.status = status;
  return err;
}

// Runs the full generate + poll flow and returns the final image URL, or
// throws a MagnificGenerationError with an HTTP status the caller can relay.
export async function runMagnificGeneration(theme: string, brand?: string): Promise<string> {
  if (!theme || !theme.trim()) {
    throw makeError("O tema digitado está vazio.", 400);
  }
  ...
  if (!apiKey) {
    ...
    throw makeError("Chave da API Magnific não configurada no servidor (.env).", 500);
  }
  ...
  if (!response.ok) {
    ...
    throw makeError(`A API do Magnific respondeu com erro (${response.status}): ${errText || response.statusText}`, response.status);
  }
  ...
  if (!taskInfo || !taskInfo.task_id) {
    ...
    throw makeError("ID da tarefa de geração de imagem não retornado pela API.", 500);
  }
  ...
  while (!isTerminalStatus(status) && pollAttempts < maxAttempts) {
    ...
    try {
      ...
      if (pollResponse.ok) {
        ...
        if (pollData && pollData.data) {
          ...
          if (String(status).toUpperCase() === "COMPLETED") {
            ...
            throw makeError("A geração de imagem falhou no processamento da API.", 500);
          }
        }
      } else {
      ...
      if (pollErr && typeof pollErr.status === "number") throw pollErr;
      console.warn(`[Magnific Server] Error during status polling: ${pollErr.message}`);
    }
  }

  if (String(status).toUpperCase() !== "COMPLETED") {
    throw makeError("Tempo limite esgotado aguardando a geração da imagem pelo Magnific.", 504);
  }
  ...
  if (!imageUrl) {
    ...
    throw makeError("Geração concluída, mas nenhuma URL de imagem foi retornada.", 500);
  }
