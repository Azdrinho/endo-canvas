...
app.post("/api/magnific/generate", async (req, res) => {
  try {
    ...
    // Keep polling while the task is NOT in a terminal state. Matching only
    // "CREATED"/"IN_PROGRESS" would abort early on any other in-flight status
    // (e.g. "PROCESSING"/"PENDING" or a different casing), yielding a spurious
    // 504 while generation is still running.
